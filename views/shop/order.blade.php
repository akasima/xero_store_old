        ...
        <div class="summary">
            <div>
                <div>
                    ...
                    배송비 <span class="shippingFee">0</span>
                </div>
                <div>
                    <span class="sum">{{number_format($summary['amount'] + $summary['shippingFee'])}}</span>
                </div>
            </div>
        </div>

        <div class="shipping">
            <input type="hidden" name="shippingType" value="{{$selectedShipping}}"/>
            <input type="hidden" name="shippingId" value=""/>
            <div class="dst-types">
                @foreach ($shippings as $info)
                    <button type="button" class="info" data="{{$info->id}}">{{$info->title}}</button>
                @endforeach
                @if ($recentShipping != null)
            ...
            <div class="shippingInfo" id="recent">
                <p>받는사람 : {{$recentShipping->recvName}}</p>
                <p>주소 : ({{$recentShipping->postcode}}) {{$recentShipping->address1}} {{$recentShipping->address2}}</p>
                <p>휴대전화 : {{$recentShipping->phone}}</p>
            </div>
            @endif
        ...
        <div>
                @foreach($payment->getMethods() as $method)
                    <label class="radio-inline">
                        <input type="radio" class="pay-method" name="payMethod" value="{{ $method->getMethodKey() }}"> {{ $method->getReadableName() }}
                    </label>
                @endforeach
                <button type="button" class="btn btn-default start-payment" data-url="{{route('rich_shop.payment.start')}}" data-callback="{{route('rich_shop.payment.complete')}}">결제</button>
        </div>
    </div>
</form>
